		...
		struct libwebsocket_context *context, struct libwebsocket *wsi)
{
	...
	while (!lws_send_pipe_choked(wsi)) {
		if (wsi->u.http.bin) {
			...
			if (n > (wsi->u.http.filelen - wsi->u.http.filepos))
				n = wsi->u.http.filelen - wsi->u.http.filepos;
			/* copy must be done, PADDING is needed before and after buffer */
			memcpy(context->service_buffer, wsi->u.http.bin + wsi->u.http.filepos, n);
		...
		if (n > 0) {
			// test: m = libwebsocket_write(wsi, context->service_buffer, n>1? n-1: n,
...
static int libwebsockets_serve_http_bin_or_file(struct libwebsocket_context *context,
			struct libwebsocket *wsi,
			const unsigned char *bin, size_t bin_size, const char* file,
...
"HTTP/1.0 200 OK\x0d\x0aServer: libwebsockets\x0d\x0a""Content-Type: %s\x0d\x0a",
	...
	p += sprintf((char *)p,
		"Content-Length: %u\x0d\x0a\x0d\x0a",
					(unsigned int)wsi->u.http.filelen);
...
LWS_VISIBLE int libwebsockets_serve_http_file(struct libwebsocket_context *context,
			struct libwebsocket *wsi, const char *file,
						       const char *content_type)
{
	return libwebsockets_serve_http_bin_or_file(context, wsi, NULL, 0, file, content_type);
}

LWS_VISIBLE int libwebsockets_serve_http_bin(struct libwebsocket_context *context,
			struct libwebsocket *wsi, const unsigned char *bin, size_t bin_size,
						       const char *content_type)
{
	return libwebsockets_serve_http_bin_or_file(context, wsi, bin, bin_size, NULL, content_type);
}
